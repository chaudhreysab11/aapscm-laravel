<x-layouts.cms>
        ...
        <section class="bg-[#f5f7fa] py-16 md:py-20">
            <div class="max-w-[1100px] mx-auto px-4">
                ...
                @if (! empty($d['why_pursue']['intro']))
                    <p class="text-[15px] md:text-[16px] text-gray-700 text-center leading-relaxed mb-10">{{ $d['why_pursue']['intro'] }}</p>
                @endif
                <ul class="space-y-4 max-w-[900px] mx-auto">
                    @foreach ($d['why_pursue']['items'] as $li)
    ...
    {{-- Key Learning Areas --}}
    @if (! empty($d['key_learning_areas']))
        <section class="bg-white py-16 md:py-20">
            <div class="max-w-[1200px] mx-auto px-4">
                <h2 class="text-[26px] md:text-[32px] font-bold text-[#14166e] uppercase leading-snug text-center mb-5">{{ $d['key_learning_areas']['heading'] }}</h2>
                @if (! empty($d['key_learning_areas']['intro']))
                    <p class="max-w-[1000px] mx-auto text-[15px] md:text-[16px] text-gray-700 text-center leading-relaxed mb-12">{{ $d['key_learning_areas']['intro'] }}</p>
                @endif
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($d['key_learning_areas']['items'] as $c)
                        <div class="bg-[#f5f7fa] rounded-lg p-6">
                            <div class="text-[36px] md:text-[42px] font-bold text-[#5cb85c] leading-none mb-3">{{ $c['number'] }}</div>
                            <h3 class="text-[17px] md:text-[18px] font-bold text-[#14166e] mb-2 leading-snug">{{ $c['title'] }}</h3>
        ...
        <section class="bg-[#f5f7fa] py-16 md:py-20">
            <div class="max-w-[1100px] mx-auto px-4">
                ...
                @if (! empty($d['who_should_pursue']['intro']))
                    <p class="text-[15px] md:text-[16px] text-gray-700 text-center leading-relaxed mb-10">{{ $d['who_should_pursue']['intro'] }}</p>
                @endif
                <ul class="space-y-4 max-w-[900px] mx-auto">
    ...
    {{-- Why Stands Out --}}
    @if (! empty($d['why_stands_out']))
        <section class="bg-white py-16 md:py-20">
            <div class="max-w-[1100px] mx-auto px-4">
                <h2 class="text-[26px] md:text-[32px] font-bold text-[#14166e] uppercase leading-snug text-center mb-12">{{ $d['why_stands_out']['heading'] }}</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    @foreach ($d['why_stands_out']['items'] as $c)
                        <div>
    ...
    @if (! empty($d['transform']))
        <section class="bg-[#14166e] py-16 md:py-20 text-white">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                <div>
                    @if (! empty($d['transform']['heading']))
                        <h2 class="text-[26px] md:text-[34px] font-bold uppercase leading-snug mb-6">{{ $d['transform']['heading'] }}</h2>
                    @endif
    ...
    {{-- Closing Line --}}
    @if (! empty($d['closing_line']))
        <section class="bg-white py-12">
            <div class="max-w-[1100px] mx-auto px-4 text-center">
                <p class="text-[15px] md:text-[17px] text-gray-700 italic leading-relaxed">{{ $d['closing_line'] }}</p>
            </div>
        </section>
    @endif
