<x-layouts.cms>
    ...
    @php
        ...
        $differences  = $data['differences_section'] ?? [];
        $recommend    = $data['recommendation_section'] ?? [];
    ...
    {{-- Empowering Professionals: heading + divider + sub-heading + paragraph | image --}}
    @if (! empty($building))
            ...
            <div class="max-w-[1200px] mx-auto px-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                    ...
                    @if (! empty($building['image']))
                        <div>
                            <img src="{{ $building['image'] }}" alt="E-commerce Administration Professionals" class="w-full h-auto" />
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="max-w-[1200px] mx-auto px-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                    @if (! empty($managers['image']))
                        <div>
                            <img src="{{ $managers['image'] }}" alt="E-commerce Management Professionals" class="w-full h-auto" />
                        </div>
                    @endif
    ...
    {{-- Differences section: heading + intro + 2 columns each with sub-heading + bullets --}}
    @if (! empty($differences))
        <section class="bg-white py-14">
            <div class="max-w-[1200px] mx-auto px-4">
                @if (! empty($differences['heading_html']))
                    <div class="text-center mb-4">
                        <h2 class="text-[26px] md:text-[34px] font-light text-[#14166e]">
                            {!! $differences['heading_html'] !!}
                        </h2>
                        <div class="w-16 h-[3px] bg-[#e74c1d] mx-auto mt-4"></div>
                    </div>
                @endif

                @if (! empty($differences['intro_html']))
                    <p class="text-center text-[16px] text-gray-700 leading-relaxed max-w-[900px] mx-auto mb-10">
                        {!! $differences['intro_html'] !!}
                    </p>
                @endif

                @if (! empty($differences['columns']))
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        @foreach ($differences['columns'] as $col)
                            <div class="bg-[#f5f7fa] rounded-lg p-6">
                                @if (! empty($col['heading_html']))
                                    <h3 class="text-[20px] md:text-[24px] font-semibold text-[#14166e] mb-4">
                                        {!! $col['heading_html'] !!}
                                    </h3>
                                @endif

                                @if (! empty($col['text_html']))
                                ...
                                @if (! empty($col['items']))
                                    <ul class="space-y-3">
                                        @foreach ($col['items'] as $it)
                                            <li class="flex items-start gap-3">
                                                @if (! empty($it['icon']))
                                                    <img src="{{ $it['icon'] }}" alt="" class="w-8 h-auto flex-shrink-0 mt-1" />
                                                @endif
                                                @if (! empty($it['desc']))
                                                    <div class="text-gray-700 leading-relaxed text-[15px]">
                                                        {!! $it['desc'] !!}
    ...
    @if (! empty($recommend) && (! empty($recommend['heading_html']) || ! empty($recommend['text_html'])))
        <section class="bg-[#f6f8fb] py-12">
            <div class="max-w-[1000px] mx-auto px-4">
                @if (! empty($recommend['heading_html']))
                    <h2 class="text-[22px] md:text-[28px] font-semibold text-[#14166e] mb-4">
                        {!! $recommend['heading_html'] !!}
                    </h2>
                @endif

                @if (! empty($recommend['text_html']))
                    <div class="prose prose-sm md:prose-base max-w-none text-gray-700 leading-relaxed">
                        {!! $recommend['text_html'] !!}
                    </div>
                @endif
