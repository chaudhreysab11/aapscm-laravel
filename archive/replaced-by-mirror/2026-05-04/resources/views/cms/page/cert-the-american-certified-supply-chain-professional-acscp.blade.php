<x-layouts.cms>
    ...
    {{-- SECTION 2 — Why Go for ACSCP® Certification? Navy bg, centered --}}
    @if (! empty($whyCert))
    ...
    {{-- SECTION 3 — About the Exam: image + caption left, heading + intro + bullets right --}}
    @if (! empty($aboutExam))
        <section class="bg-white py-14">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
                <div class="space-y-5">
                    @if (! empty($aboutExam['image']))
                        <div class="flex justify-center">
                            <img src="{{ $aboutExam['image'] }}" alt="{{ $aboutExam['image_caption'] ?? '' }}"
    ...
    {{-- SECTION 4+5+6 — AI Applications: bridging intro + 4 image-box cards + closing paragraph --}}
    @if (! empty($aiApps))
        <section class="bg-[#f5f7fa] py-14">
            <div class="max-w-[1200px] mx-auto px-4">
                @if (! empty($aiApps['intro']))
                    <p class="text-[15px] md:text-[16px] text-gray-700 leading-relaxed max-w-[1000px] mx-auto text-center mb-10">
                        {{ $aiApps['intro'] }}
                    </p>
                @endif
                @if (! empty($aiApps['cards']))
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        @foreach ($aiApps['cards'] as $card)
    ...
    {{-- SECTION 7 — Foundational AI in Supply Chain Management: navy marquee --}}
    @if (! empty($aiIntro))
        <section class="bg-[#14166e] py-16 text-white">
            <div class="max-w-[1100px] mx-auto px-4 text-center">
                <h2 class="text-[26px] md:text-[34px] font-bold uppercase mb-6 leading-snug">
    ...
    {{-- SECTION 8 — Who Would Benefit: image left, heading + body right --}}
    ...
    {{-- SECTION 9 — Why Would You Benefit: heading + body left, image right --}}
    @if (! empty($whyBenefit))
        <section class="bg-[#f5f7fa] py-14">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                <div>
    ...
    {{-- SECTION 10 + 11 — Exam Details table + Download Flyer CTA --}}
