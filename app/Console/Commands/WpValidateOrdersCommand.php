...
class WpValidateOrdersCommand extends Command
{
    protected $signature = 'wp:validate-orders
                            {--file=database/import/wordpress/orders.csv : WordPress orders CSV export}
                            {--users=database/import/wordpress/users-customer.csv : WordPress users/customers CSV export}
    ...
    protected $description = 'Validate WordPress orders CSV without importing orders or order items';

    public function handle(WordPressCsvValidationService $validator): int
    {
        $result = $validator->validateOrders(
            (string) $this->option('file'),
            (string) $this->option('users'),
        ...
        if ((bool) $this->option('write-flags')) {
            $written = $validator->writeFlags($result['issues'], (bool) $this->option('clear-existing'), 'WpValidateOrders');
