...
<head>
    ...
    {{-- GrapesJS CSS is bundled into visual-editor.js output --}}
    @vite(['resources/css/app.css', 'resources/js/visual-editor.js'])
...
<div class="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 h-12 bg-[#0f1117] border-b border-gray-800">
    ...
    <div class="flex items-center gap-3">
        ...
        <span class="text-sm text-gray-300 font-medium truncate max-w-[240px]">{{ $page->title }}</span>

        @if($page->template === 'gjs')
            <span class="text-xs bg-indigo-600 text-white px-2 py-0.5 rounded">Block Builder</span>
        @else
            <span class="text-xs bg-gray-600 text-white px-2 py-0.5 rounded">{{ $page->template }}</span>
        @endif
    </div>
...
{{-- ═══════════════════════════════════════════════════════════════
     MAIN LAYOUT  (top-bar = 48px, so pt-12)
══════════════════════════════════════════════════════════════════ --}}
<div class="flex h-full pt-12">
    ...
    <div class="w-56 flex-shrink-0 bg-[#1e2235] border-l border-gray-800 flex flex-col overflow-hidden">
        ...
        {{-- Trait manager (component settings like href, src, alt) --}}
...
window.BUILDER_CONFIG = {
    pageId:      {{ $page->id }},
    pageContent: @json($page->content ?? ''),
    gjsData:     @json(data_get($page->blocks ?? [], 'gjs', null) ?? new stdClass()),
    savePath:    '{{ route('builder.save', $page) }}',
