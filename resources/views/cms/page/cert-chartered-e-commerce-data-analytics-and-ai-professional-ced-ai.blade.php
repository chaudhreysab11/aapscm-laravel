<x-layouts.cms>
    ...
    {{-- Who Should Pursue (image left, image-box list right) --}}
    @if (! empty($d['who_should_pursue']))
        <section class="bg-white py-16 md:py-20">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
                @if (! empty($d['who_should_pursue']['image']))
                    <div><img src="{{ $d['who_should_pursue']['image'] }}" alt="{{ $d['who_should_pursue']['heading'] ?? '' }}" class="w-full h-auto rounded-md" loading="lazy"></div>
                @endif
                <div>
                    <h2 class="text-[26px] md:text-[32px] font-bold text-[#14166e] uppercase leading-snug mb-4">{{ $d['who_should_pursue']['heading'] }}</h2>
                    @if (! empty($d['who_should_pursue']['intro']))
                        <p class="text-[15px] md:text-[16px] text-gray-700 leading-relaxed mb-6">{{ $d['who_should_pursue']['intro'] }}</p>
                    @endif
                    <ul class="space-y-5">
                        @foreach ($d['who_should_pursue']['items'] as $li)
                            <li class="flex gap-4 items-start">
                                @if (! empty($li['image']))
                                    <img src="{{ $li['image'] }}" alt="{{ $li['title'] }}" class="w-12 h-12 object-contain flex-shrink-0">
                                @endif
                                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    @foreach ($d['eligibility']['items'] as $li)
                        <div class="flex gap-4 items-start">
                            @if (! empty($li['image']))
                                <img src="{{ $li['image'] }}" alt="" class="w-12 h-12 object-contain flex-shrink-0">
                            @endif
