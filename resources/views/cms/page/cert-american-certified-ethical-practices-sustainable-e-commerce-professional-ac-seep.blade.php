<x-layouts.cms>
        ...
        <section class="bg-[#f5f7fa] py-16 md:py-20">
            <div class="max-w-[1100px] mx-auto px-4">
                ...
                @if (! empty($d['learning_outcomes']['intro']))
                    <p class="text-[15px] md:text-[16px] text-gray-700 text-center leading-relaxed mb-10">{{ $d['learning_outcomes']['intro'] }}</p>
                @endif
                <ul class="space-y-4 max-w-[900px] mx-auto">
                    @foreach ($d['learning_outcomes']['items'] as $li)
                        <li class="flex gap-3 text-[15px] md:text-[16px] text-gray-700 leading-relaxed">
                            @if (! empty($d['learning_outcomes']['check_icon']))
                                <img src="{{ $d['learning_outcomes']['check_icon'] }}" alt="" class="w-6 h-6 object-contain flex-shrink-0 mt-[2px]">
                            @else
    ...
    @if (! empty($d['benefits']))
        <section class="bg-[#f5f7fa] py-16 md:py-20">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                @if (! empty($d['benefits']['image']))
                    <div><img src="{{ $d['benefits']['image'] }}" alt="{{ $d['benefits']['heading'] ?? '' }}" class="w-full h-auto rounded-md" loading="lazy"></div>
                @endif
                <div>
                    <h2 class="text-[26px] md:text-[32px] font-bold text-[#14166e] uppercase leading-snug mb-6">{{ $d['benefits']['heading'] }}</h2>
                    <ul class="space-y-4">
