<x-layouts.cms>
                ...
                @foreach ($cards as $card)
                    ...
                    <div class="bg-white rounded-lg shadow-[rgba(100,100,111,0.12)_0px_4px_14px_0px] p-6 flex flex-col text-center hover:-translate-y-1 transition-transform">
                        ...
                        @if (! empty($card['heading_html']))
                            <h2 class="text-[28px] md:text-[32px] font-semibold text-[#14166e] leading-snug mb-3 min-h-[56px]">
                                {!! $card['heading_html'] !!}
                            </h2>
                        @endif
    ...
    {{-- Bottom CTA: Select Your Training or Purchase the Test --}}
    @if (! empty($bottom) && (! empty($bottom['heading_html']) || ! empty($bottom['items'])))
        <section class="bg-white py-14">
            <div class="max-w-[1100px] mx-auto px-4 text-center">
                @if (! empty($bottom['heading_html']))
                ...
                @if (! empty($bottom['text_html']))
                    <p class="text-[16px] md:text-[18px] text-gray-700 leading-relaxed max-w-[900px] mx-auto mb-8">
                        {!! $bottom['text_html'] !!}
                    </p>
                @endif

                @if (! empty($bottom['items']))
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-[900px] mx-auto mb-8">
                        @foreach ($bottom['items'] as $it)
                            <div class="bg-[#f6f8fb] rounded-lg p-6 flex items-start gap-4 text-left">
                                @if (! empty($it['icon']))
                ...
                {{-- @if (! empty($bottom['cta_label']))
                    <a href="{{ $bottom['cta_href'] ?: '#' }}"
                       class="inline-flex items-center justify-center gap-2 bg-[#14166e] hover:bg-[#1e2199] text-white font-semibold text-base px-7 py-3 rounded-full transition-colors">
                        {{ $bottom['cta_label'] }}
                    </a>
                @endif --}}
            </div>
