    ...
    @if (! empty($d['why_aapscm']))
        <section class="bg-white py-16 md:py-20">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10">
                <div>
                    <h2 class="text-[26px] md:text-[32px] font-bold text-[#14166e] uppercase leading-snug mb-5">{{ $d['why_aapscm']['heading'] }}</h2>
                    @if (! empty($d['why_aapscm']['subheading']))
                        <h3 class="text-[18px] md:text-[20px] font-bold text-[#14166e] mb-4">{{ $d['why_aapscm']['subheading'] }}</h3>
                    @endif
