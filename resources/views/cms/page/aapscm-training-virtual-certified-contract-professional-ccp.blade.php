<x-layouts.cms>
    ...
    @php
        ...
        $overviewBand   = $page->page_data['overview_band']   ?? [];
        $whyCert        = $page->page_data['why_cert']        ?? [];
        $whyMatters     = $page->page_data['why_matters']     ?? [];
        $whyMatters2    = $page->page_data['why_matters_2']   ?? [];
    ...
    {{-- SECTION — Overview band: 2-col with photo background; text in dark navy card on the left --}}
    @if (! empty($overviewBand))
        <section class="py-0" style="background-image: url('{{ asset('storage/cms-images/2026/03/slider-img.jpg') }}'); background-position: center right; background-repeat: no-repeat; background-size: cover;">
            <div class="max-w-[1200px] mx-auto px-4 py-16 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                <div class="bg-[#14213a]/90 text-white p-8 md:p-10 rounded-md shadow-[rgba(0,0,0,0.25)_0px_8px_28px_0px]">
                    <h2 class="text-[26px] md:text-[34px] text-white leading-snug mb-6">
                        @php
                            $ovHeading = (string) ($overviewBand['heading'] ?? '');
                            $ovParts = preg_split('/\s+/', trim($ovHeading), 2);
                        @endphp
                        @if (count($ovParts) === 2)
                            <span class="font-light">{{ $ovParts[0] }}</span>
                            <span class="font-bold">{{ $ovParts[1] }}</span>
                        @else
                            <span class="font-bold">{{ $ovHeading }}</span>
                        @endif
                    </h2>
                    @foreach (($overviewBand['paragraphs'] ?? []) as $p)
                        <p class="text-[14px] md:text-[15px] text-gray-200 leading-relaxed mb-4 last:mb-0">
                            {{ $p }}
                        </p>
                    @endforeach
                </div>
    ...
    {{-- SECTION — Why ... Matters #2: same shape as why_matters, white background --}}
    @if (! empty($whyMatters2))
        <section class="bg-white py-14">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                @if (! empty($whyMatters2['image']))
                    <div class="flex justify-center">
                        <img src="{{ $whyMatters2['image'] }}" alt="{{ $whyMatters2['heading'] ?? '' }}"
                             class="w-full max-w-[520px] h-auto" />
                    </div>
                @endif
                <div>
                    ...
                    @if (! empty($whyMatters2['intro']))
                        <p class="text-[15px] md:text-[16px] text-gray-700 leading-relaxed mb-4">
                            {{ $whyMatters2['intro'] }}
                        </p>
                    @endif
                    @if (! empty($whyMatters2['items_a']))
                        <ul class="space-y-3 text-[15px] md:text-[16px] text-gray-700 leading-relaxed mb-5">
                            @foreach ($whyMatters2['items_a'] as $li)
                                <li class="flex gap-3">
                                    <span class="text-[#5cb85c] font-bold mt-[2px]">&#10003;</span>
                                    <span>{{ $li }}</span>
                                </li>
        ...
        <section class="bg-white py-14">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-8">
                @foreach ($outcomes['cards'] as $c)
                    <div class="bg-[#f5f7fa] rounded-lg shadow-[rgba(100,100,111,0.12)_0px_4px_18px_0px]">
                        @if (! empty($c['image']))
                            <div class="mb-4">
                                <img src="{{ $c['image'] }}" alt="{{ $c['heading'] ?? '' }}"
                                     class="w-auto max-w-[200px] h-auto mx-auto" />
                            </div>
                        @endif
                        <h3 class="text-[18px] md:text-[22px] px-6 font-bold text-[#14166e] leading-snug mb-1">
                            {{ $c['heading'] }}
                        </h3>
                        @if (! empty($c['subheading']))
                            <p class="text-[14px] md:text-[15px] px-6 font-semibold text-gray-600 mb-3">
                                {{ $c['subheading'] }}
                            </p>
                        @endif
                        @if (! empty($c['intro']))
                            <p class="text-[15px] md:text-[16px] px-6 text-gray-700 leading-relaxed mb-3">
                                {{ $c['intro'] }}
                            </p>
                        @endif
                        @if (! empty($c['items']))
                            <ul class="space-y-2 text-[14px] md:text-[15px] px-6 text-gray-700 leading-relaxed">
