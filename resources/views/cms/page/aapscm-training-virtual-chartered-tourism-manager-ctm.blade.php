<x-layouts.cms>
    ...
    @if (! empty($intro))
        <section class="bg-white py-14">
            <div class="max-w-[1100px] mx-auto px-4 text-center">
                ...
                @endif
                @if (! empty($intro['body_paragraphs']))
                    @foreach ($intro['body_paragraphs'] as $paragraph)
                        <p class="text-[15px] md:text-[16px] text-gray-700 leading-relaxed{{ ! $loop->first ? ' mt-4' : '' }}">
                            {{ $paragraph }}
                        </p>
                    @endforeach
                @endif
        ...
        <section class="bg-[#f5f7fa] py-14">
            <div class="max-w-[1200px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                ...
                <div>
                    ...
                    @if (! empty($whyCert['body_paragraphs']))
                        @foreach ($whyCert['body_paragraphs'] as $para)
                            <p class="text-[15px] md:text-[16px] text-gray-700 leading-relaxed mb-4 last:mb-0">{{ $para }}</p>
                        @endforeach
                    @elseif (! empty($whyCert['body']))
                        <p class="text-[15px] md:text-[16px] text-gray-700 leading-relaxed">
                            {{ $whyCert['body'] }}
                        </p>
                    @endif
    ...
    {{-- SECTION 3.5 — Modules grid (Program Structure and Modules) --}}
    @if (! empty($modules) && ! empty($modules['items']))
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($modules['items'] as $item)
                        <div class="bg-white text-gray-700 rounded-lg p-6 shadow-[rgba(0,0,0,0.18)_0px_6px_18px_0px] flex flex-col">
                            @if (! empty($item['icon']))
                                <img src="{{ $item['icon'] }}" alt="" class="w-12 h-12 mb-4" />
                            @endif
                            <h3 class="text-[16px] md:text-[18px] font-bold text-[#14166e] mb-2 leading-snug">
                                {{ $item['title'] }}
                            </h3>
                            <p class="text-[14px] md:text-[15px] leading-relaxed">{{ $item['desc'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
